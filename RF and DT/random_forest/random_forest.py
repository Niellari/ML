...
from random_forest.sampler import FeatureSampler, ObjectSampler, BaseSampler
...
class RandomForest:
    ...
    def predict_proba(self, x):
        ...
        probas = np.zeros((x.shape[0], self.estimators[0].n_classes_))
        for estimator, indices in zip(self.estimators, self.indices):
            x_sample = x[:, indices]  # Оставляем только выбранные признаки
...
rm = RandomForest(base_estimator = DecisionTreeClassifier, object_sampler = ObjectSampler(),feature_sampler=FeatureSampler() )

X, y = make_classification(n_samples=20,    # количество объектов
...
n_objects = X.shape[1]
max_samples = 1
n_samples = max_samples * n_objects
